...
parallel<-psych::fa.parallel(coglongdata[,allefvars],n.iter=1000,fm="ml",quant=.95)
save(parallel,file="~/Library/Mobile\ Documents/com~apple~CloudDocs/Projects/R03_behavioral/NKI/Data/parallelsavefactor.Rdata") 
load(file="~/Library/Mobile\ Documents/com~apple~CloudDocs/Projects/R03_behavioral/NKI/Data/parallelsavefactor.Rdata") ##already run load in to save computation time
...
favals<-fa(coglongdata[,allefvars],nfactors=3,rotate ="bifactor",fm="ml",scores="tenBerge")
favalsforeigvis<-fa(coglongdata[,allefvars],nfactors=length(allefvars),rotate ="bifactor",fm="ml",scores="tenBerge")
####
...
ggeigs<-lunaize(ggeigs)+theme(legend.position = "top",legend.title = element_blank())+xlab("Factor #")+ylab("Eigenvalue")+geom_hline(yintercept = mean(eigvals$eigvals),linetype="solid")+
  ...
  geom_text(data=screedata,aes(x=7,y=2.5,label=finlabel),lineheight = .9,size = 6)+
  scale_x_continuous(breaks=seq(1,max(eigvals$index)))
ggsave(ggeigs,file="~/Library/Mobile\ Documents/com~apple~CloudDocs/Projects/R03_behavioral/Figures/Factorfigs/NKIeigsfig.pdf",width=6,height=5)
...
ggsave(fullpanel,file="~/Library/Mobile\ Documents/com~apple~CloudDocs/Projects/R03_behavioral/Figures/Factorfigs/NKIeigwithloading.panel.agesesid.pdf",width=8,height=5)
...
write.table(corssaveclean,file="~/Library/Mobile\ Documents/com~apple~CloudDocs/Projects/R03_behavioral/Figures/Factorfigs/NKIcorrs.csv",row.names=FALSE,col.names=FALSE,sep=",",quote=FALSE)
...
save(alleigssaves,file="~/Library/Mobile\ Documents/com~apple~CloudDocs/Projects/R03_behavioral/Figures/Factorfigs/allfactordataNKI.Rdata")
